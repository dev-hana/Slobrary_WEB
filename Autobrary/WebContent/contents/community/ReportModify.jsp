...
<jsp:include page="/Top.jsp" flush="false" />
	<%
		ReportBean reportBean = reportMgr.getReport("3"); //예시
	%>
		...
		<form method="post" action="GoModifyProc.jsp">
			...
			<div class="title mb-2">
				<div class="input-group">
									<!-- 수정할 도서명 -->
				<input type="text" value="수정할 도서명" class="form-control input-title mr-3" name="book" disabled="disabled">
				<span class="input-group-text">제목</span>
				<input type="text" class="form-control input-title" name="title" value="<%=reportBean.getName() %>" required>
				</div>
			</div>
			<div>
...
$(document).ready(function() {
	  ...
	  //원본글 삽입 코드
	  $('#summernote').summernote('code', '<%=reportBean.getContent()%>');
	});
</script>
</body>
</html>